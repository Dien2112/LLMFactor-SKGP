...
```
# arXiv:2406.10811v1 [cs.CL] 16 Jun 2024


ods encounter various limitations. We propose
...
ing, we introduce a new frameworkLLMFactor,
which derives factors from LLMs throughSequen-
...
Figure 2: LLMFactor: From Sequential Knowledge-Guided Prompting (SKGP) to factor analysis in the stock market.
...
```
3.1 Task Definition
For a given stock, denoted asstocktarget, we
consider its associated news published on the
target prediction datedatetarget, referred to as
newstarget, and its historical stock price sequence
P={P 1 ,P 2 ,...,Pt}, wheretrepresents the win-
...
a series of stock movements,Pˆ={Pˆ 1 ,Pˆ 2 ,...,Pˆt}.
In this series,Pˆi= 1indicates a rise in the stock
price fromPi− 1 toPi, whereasPˆi= 0indicates
a fall. Our objective is to predictPˆt+1given
datetarget,newstarget, andPˆ.
```
```
...
Nvidia (NVDA) as our examplestocktarget, the
initial phase involves matching the stock with rel-
evant news and acquiring background knowledge.
```

LetS = {(Ci,Ti,Ii) | i ∈ N,i≤ n}be the
stock list, where each tuple(Ci,Ti,Ii)consists
of a companyCi, its ticker symbolTi, and its
industryIi. These tuples are collected from the
NYSE and NASDAQ exchanges, withnbeing
a positive integer representing the total number
of such tuples. We matchSwithnewstargetto
obtainstockmatch={S∩newstarget}. Follow-
ing this, we prompt LLMs to obtain relations be-
tweenstocktargetandstockmatch. The prompting
method is defined asLLM:String→String,
where the input is aRelationTemplate“Please
fill in the blank: stocktargetandstockmatchare
most likely in a ___ relationship,” and the output is
the type of relationship, denoted asrelation.
This method of deriving background knowledge
aboutnewstargetsignificantly improves our un-
derstanding of the news content. For example, if
stocktargetandstockmatchare identified as com-
petitors, information aboutstockmatchcould poten-
tially have a negative impact onstocktarget. Fur-
...
tors fromnewstarget. The importance of these
...
of LLMs. Considering the topk factors we
aim to extract for the given stocktarget and
newstarget, the prompting method is described
asLLM(FactorTemplate) = factor, where
FactorTemplateis a structured sentence: “Please
```
```
extract the topkfactors that may affect the stock
price ofstocktargetfrom the following news,” fol-
lowed bynewstarget, and the output is the factors
...
sequence of stock movements denoted byPˆ =
{Pˆ 1 ,Pˆ 2 ,...,Pˆt}, we introduce a function where
f(Pˆi = 0) is assigned the value “fell” and
f(Pˆi= 1)is assigned “rose,” thereby convertingPˆ
into a sequence of outcomesTextMovement=
{f(Pˆi)|i∈ { 1 , 2 ,...,t},f(Pˆi) =“fell” ifPˆi=
0 and “rose” ifPˆi= 1}. Given the textual stock
movement sequenceTextMovementand its date
seriesdate= {date 1 ,date 2 ,...,datet}, the past
stock price movements are transformed into a
TimeTemplate, structured as “On datei, the
stock price ofstocktargetf(Pˆi).”
Subsequently, we construct aPriceTemplate
...
cluding instruction, “Ondatei, the stock price of
stocktargetwill ___.” By integrating therelation,
factor,TimeTemplate, andPriceTemplate,
we articulate the prompting method as
LLM(relation,factor,TimeTemplate,
PriceTemplate) =prediction.Theprediction
...
```
Statistics StockNet CMIN-US CMIN-CN EDT
...
Data Size 19,318 83,553 198,781 54,
Stock Market US US CN US
Stock Number 87 110 300 4,
Date Range 2014-01-01 to 2016-01-01 2018-01-01 to 2021-12-31 2018-01-01 to 2021-12-31 2020-03-01 to 2021-05-
```
```
...
tweets and historical price data from 2014-01-
...
tweets and historical price data from 2018-01-
...
(^1) A market capitalization-weighted index crafted to mir-
...
- TFIDF(Sparck Jones, 1972): A traditional unsu-
...
- RoBERTa: Applies Financial RoBERTa^2 for
...
- FinBERT: A BERT model fine-tuned for finan-
    ...
    English (Araci, 2019) and Chinese versions^3.
...
```
(^2) https://huggingface.co/soleimanian/
financial-roberta-large-sentiment
(^3) https://huggingface.co/bardsai/
...
follows: LetPOSbe the set of positive keyphrases
andNEGbe the set of negative keyphrases. For
a piece of textT, the scoreSis computed as:
S=σ
X
p∈P OS
I(p∈T)−
...
!
(3)
whereσ(x)is the sigmoid function defined as
σ(x) = 1+^1 e−x, andI(·)is an indicator function
that equals 1 if the condition is true and 0 other-
wise. The conditionp∈T(orn∈T) denotes the
presence of a positive (or negative) keyphrase in
the textT. For sentiment analysis, we predict the
...
datasets but exclude the use of theTimeTemplate
...
preview through the API^4 for factor-based methods.
The parameters are set as follows: 1) the window
sizetis 5; 2) the number of keyphrases and factors
kis 5; 3) the batch size is 64 for BERT series mod-
...
2.9%,0.4%,11%, and4.8%in MCC across the
...
```
Method Model ACC.StockNetMCC. ACC.CMIN-USMCC. ACC.CMIN-CNMCC. ACC.EDTMCC.
```
```
Keyphrase-based
```
```
Promptrank(ACL2023) 51.24 0.010 53.28 0.001 50.21 -0.003 51.78 0.
KeyBERT 51.95 0.012 53.40 0.009 50.23 -0.004 51.84 0.
YAKE 51.91 0.005 53.13 0.001 50.20 0.001 51.88 0.
TextRank 51.00 0.003 53.99 0.060 50.38 0.006 51.76 0.
TopicRank 51.92 0.008 53.75 0.034 50.26 -0.002 51.80 0.
SingleRank 50.32 0.005 53.33 0.004 50.29 0.002 51.85 0.
TFIDF 51.86 0.001 53.71 0.018 50.27 -0.002 51.86 0.
```
```
Sentiment-based
```
```
EDT(ACL2021) 40.31 -0.066 49.86 -0.004 40.00 0.021 75.67 0.
FinGPT 54.91 0.083 59.98 0.182 55.78 0.120 53.86 0.
GPT-4-turbo 53.56 0.060 64.61 0.284 56.94 0.135 55.37 0.
GPT-4 53.88 0.062 62.18 0.260 56.96 0.136 50.94 0.
GPT-3.5-turbo 52.31 0.044 56.10 0.156 56.68 0.124 54.34 0.
RoBERTa 54.46 0.088 57.75 0.138 52.24 0.064 53.66 0.
FinBERT 55.42 0.111 58.26 0.158 55.98 0.121 54.98 0.
Time-based StockNetCMIN(ACL 2023)( ACL 2018) 62.6958.23 0.2090.081 53.4352.46 0.0460.022 55.2854.53 0.1110.045 -- --
```
```
Factor-based(ours)
```
```
LLMFactorGP T− 4 −turbo 65.81 0.228 61.71 0.228 60.59 0.245 59.09 0.
LLMFactorGP T− 4 66.32 0.238 65.26 0.284 57.16 0.196 60.83 0.
LLMFactorGP T− 3. 5 −turbo 57.59 0.145 66.42 0.288 56.11 0.139 58.11 0.
```
...
+Factor 58.04 0.166 61.68 0.241 55.71 0.119 55.93 0.
+Factor+Relation 63.24 0.203 64.46 0.267 57.96 0.194 59.35 0.
```
Table 3: The ablation study results for LLMFactor, summarizing average values acrossLLMFactorGP T− 4 −turbo,
LLMFactorGP T− 4 , andLLMFactorGP T− 3. 5 −turbo. The ACC. is expressed in percentages (%), with the highest
...
ally, sentiment-based methods demonstrate varying
...
tionalfactorsextracted, and the third layer fur-
ther includesrelationsobtained. We average the
scores for ACC and MCC fromLLMGP T− 4 −turbo,

```
LLMGP T− 4 , andLLMGP T− 3. 5 −turbo, as shown
...
ofFactorTemplate, as shown in Appendix C.
4.6.2 Case Study
...
their reviews. This work was supported by JST
SPRING, Grant Number JPMJSP2108, and JST-
PRESTO, Grant Number JPMJPR2267.

## References

```
Noujoud Ahbali, Xinyuan Liu, Albert Nanda, Jamie
Stark, Ashit Talukder, and Rupinder Paul Khandpur.
```
2022. Identifying Corporate Credit Risk Sentiments
from Financial News. InProceedings of the 2022
Conference of the North American Chapter of the
Association for Computational Linguistics: Human
Language Technologies: Industry Track, pages 362–
370, Hybrid: Seattle, Washington + Online. Associa-
...
1: Long Papers), pages 6313–6326, Dublin, Ireland.
Association for Computational Linguistics.
Dogu Araci. 2019. Finbert: Financial sentiment analy-
sis with pre-trained language models.arXiv preprint
arXiv:1908.10063.
Astrid Bertrand, James R Eagan, and Winston Maxwell.
2023. Questioning the ability of feature-based ex-
planations to empower non-experts in robo-advised
financial decision-making. InProceedings of the

```
2023 ACM Conference on Fairness, Accountability,
and Transparency, pages 943–958.
Florian Boudin. 2016. pke: an open source python-
based keyphrase extraction toolkit. InProceedings of
COLING 2016, the 26th International Conference on
Computational Linguistics: System Demonstrations,
pages 69–73, Osaka, Japan. The COLING 2016 Or-
ganizing Committee.
Adrien Bougouin, Florian Boudin, and Béatrice Daille.
```
2013. TopicRank: Graph-based topic ranking for
keyphrase extraction. InProceedings of the Sixth
...
tions on Information Systems (TOIS), 37(2):1–30.
Maarten Grootendorst. 2020. Keybert: Minimal key-
word extraction with bert.
Nate Gruver, Marc Anton Finzi, Shikai Qiu, and An-
drew Gordon Wilson. 2023. Large language mod-
els are zero-shot time series forecasters. InThirty-
seventh Conference on Neural Information Process-
ing Systems.
Pei-Yi Hao, Chien-Feng Kung, Chun-Yang Chang, and
Jen-Bing Ou. 2021. Predicting stock price trends
based on financial news articles and using a novel
twin support vector machine with fuzzy hyperplane.
Applied Soft Computing, 98:106806.
Bruno Miranda Henrique, Vinicius Amorim Sobreiro,
and Herbert Kimura. 2019. Literature review: Ma-
chine learning techniques applied to financial mar-
ket prediction. Expert Systems with Applications,
124:226–251.
Yi-Ling Hsu, Yu-Che Tsai, and Cheng-Te Li. 2021.
Fingat: Financial graph attention networks for
recommending top-k k profitable stocks. IEEE
Transactions on Knowledge and Data Engineering,
35(1):469–481.


Aobo Kong, Shiwan Zhao, Hao Chen, Qicheng Li,
Yong Qin, Ruiqi Sun, and Xiaoyan Bai. 2023.
PromptRank: Unsupervised keyphrase extraction us-
ing prompt. InProceedings of the 61st Annual Meet-
ing of the Association for Computational Linguistics
(Volume 1: Long Papers), pages 9788–9801, Toronto,
...
Jingjing Xu, and Qi Su. 2021. Modeling the stock
relation with graph network for overnight stock move-
ment prediction. InProceedings of the twenty-ninth
international conference on international joint con-
ferences on artificial intelligence, pages 4541–4547.
...
for commonsense reasoning. InProceedings of the
...
Andrew W Lo. 2004. The adaptive markets hypothesis:
...
prediction. InProceedings of the 61st Annual Meet-
ing of the Association for Computational Linguis-
tics (Volume 1: Long Papers), pages 12164–12176,
Toronto, Canada. Association for Computational Lin-
guistics.

Rada Mihalcea and Paul Tarau. 2004. TextRank: Bring-
ing order into text. InProceedings of the 2004 Con-
...
Jigar Patel, Sahil Shah, Priyank Thakkar, and Ketan
Kotecha. 2015. Predicting stock and stock price in-
dex movement using trend deterministic data prepa-
ration and machine learning techniques.Expert sys-
tems with applications, 42(1):259–268.

```
Karen Sparck Jones. 1972. A statistical interpretation
of term specificity and its application in retrieval.
Journal of documentation, 28(1):11–21.
Allan Timmermann and Clive WJ Granger. 2004. Ef-
ficient market hypothesis and forecasting.Interna-
tional Journal of forecasting, 20(1):15–27.
Manuel R Vargas, Carlos EM Dos Anjos, Gustavo LG
Bichara, and Alexandre G Evsukoff. 2018. Deep
leaming for stock market prediction using techni-
cal indicators and financial news articles. In 2018
...
keyphrase extraction. InProceedings of the 22nd
International Conference on Computational Linguis-
tics (Coling 2008), pages 969–976, Manchester, UK.
Coling 2008 Organizing Committee.
Xingchen Wan, Jie Yang, Slavi Marinov, Jan-Peter Cal-
liess, Stefan Zohren, and Xiaowen Dong. 2021. Sen-
timent correlation in financial news networks and
associated market movements. Scientific reports,
11(1):3062.
Heyuan Wang, Tengjiao Wang, and Yi Li. 2020. In-
corporating expert-based investment opinion signals
in stock prediction: A deep learning framework. In
Proceedings of the AAAI Conference on Artificial
Intelligence, volume 34, pages 971–978.
...
Yan-Ze Wu, Junyu Lu, Xinyu Zhu, Weifeng Chen,
Ting-Ting Han, Kunhao Pan, Rui Wang, Hao Wang,
Xiaojun Wu, Zhong Zeng, Chong-An Chen, Ruyi
Gan, and Jiaxing Zhang. 2022. Fengshenbang 1.0:
Being the foundation of chinese cognitive intelli-
gence.ArXiv, abs/2209.02970.
...
Ming Xu. 2023. pke_zh: Python keyphrase extraction
toolkit for chinese.
Yumo Xu and Shay B. Cohen. 2018. Stock movement
prediction from tweets and historical prices. InPro-
ceedings of the 56th Annual Meeting of the Associa-
tion for Computational Linguistics (Volume 1: Long
Papers), pages 1970–1979, Melbourne, Australia. As-
...
```
```
Jingming Xue, En Zhu, Qiang Liu, and Jianping Yin.
```
2018. Group recommendation based on financial so-
cial network for robo-advisor.IEEE Access, 6:54527–
54535.

```
Xinli Yu, Zheng Chen, and Yanbin Lu. 2023. Harness-
ing LLMs for temporal data - a study on explainable
financial time series forecasting. InProceedings of
the 2023 Conference on Empirical Methods in Nat-
ural Language Processing: Industry Track, pages
739–753, Singapore. Association for Computational
Linguistics.
```
```
...
event-driven trading. InFindings of the Association
...
FinBERT bardsai/finance-sentiment-zh-baseProsusAI/finbert 438MB409MB
RoBERTa IDEA-CCNL/Erlangshen-Roberta-110M-Sentimentsoleimanian/financial-roberta-large-sentiment 1.42GB409MB
FinGPT oliverwang15/FinGPT_ChatGLM2_Sentiment_Instruction_LoRA_FTFinGPT/fingpt-sentiment_llama2-13b_lora 14.36GB7.88GB
...
LLMFactorGP T− 4 −turbo, LLMFactorGP T− 4 ,
andLLMFactorGP T− 3. 5 −turbo, are showcased in
...
```
Step EN-Template
Step1 Please fill in the blank and return a complete sentence:stocktargetandstockmatchare most likely in a ___ relationship.
Step2 Please extract the topkfactors that may affect the stock price ofstocktargetfrom the following news.
```
```
Step
```
...
These are the main factors that may affect this stock’s price recently:f actor.
These are the connections between the companies that have appeared in the news:relation.
Ondatei− 5 , the stock price ofstocktargetf(Piˆ− 5 ).
Ondatei− 4 , the stock price ofstocktargetf(Piˆ− 4 ).
Ondatei− 3 , the stock price ofstocktargetf(Piˆ− 3 ).
Ondatei− 2 , the stock price ofstocktargetf(Piˆ− 2 ).
Ondatei− 1 , the stock price ofstocktargetf(Piˆ− 1 ).
Ondatei, the stock price ofstocktargetwill ___.
```
```
...
```
Step CN-Template
Step1 请填空并返回完整的句子:stocktarget 和stockmatch 最可能是___关系。
Step2 请从以下新闻中提取可能影响stocktarget 股价的前k个因素。
```
```
Step
```
...
在datei-5, stocktarget 的股价f(Pˆi-5)。
在datei-4, stocktarget 的股价f(Pˆi-4)。
在datei-3, stocktarget 的股价f(Pˆi-3)。
在datei-2, stocktarget 的股价f(Pˆi-2)。
在datei-1, stocktarget 的股价f(Pˆi-1)。
在datei, stocktarget 的股价将___。
```
```
Table 6: SKGP Templates in Chinese
```
Factor Template LLMFactorACC. GP TMCC.−^4 −turboLLMFactorACC. MCC.GP T−^4 LLMFactorACC. GP TMCC.−^3.^5 −turbo
Please extract the topkfactors that may affect the stock price ofstocktargetfrom the following news 61.71 0.228 65.26 0.284 66.42 0.
Please identify the primary topkfactors influencingstocktarget’s stock price based on the news provided 66.98 0.292 64.83 0.246 65.26 0.
Please analyze the provided news and pinpoint the topkmajor factors impacting the stock price ofstocktarget65.29 0.295 69.21 0.312 66.56 0.

```
Table 7: Experimental Results for Different Factor Templates in English
```
```
Factor Template LLMFactorACC. GP TMCC.−^4 −turbo LLMFactorACC. MCC.GP T−^4 LLMFactorACC. GP TMCC.−^3.^5 −turbo
请从以下新闻中提取可能影响stocktarget股价的前k个因素 60.59 0.245 57.16 0.196 56.11 0.
根据提供的新闻，请识别出影响stocktarget股价的主要k个因素 57.92 0.147 64.79 0.109 65.13 0.
请分析所提供的新闻并找出影响stocktarget股价的前k个主要因素 64.29 0.160 59.33 0.053 59.49 0.
